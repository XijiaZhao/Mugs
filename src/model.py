...
class Instance_Superivsion_Head(nn.Module):
    ...
    """
    '''
    question: 1. the existence of batch normalization:mlp.append(nn.BatchNorm1d(dim2)):\
    means to do the batchnorm on the last layer
    2. the way to initialize the network
    '''
    ...
    def __init__(
        ...
        nlayers = max(nlayers, 1) # Ensure nlayers is at least 1

        # whether do the normalization before prediction
        ...
        self.apply(self._init_weights)# Initialize weights of the network

        # Initialize the predictor head as None
        self.predictor = None
        # If pred_hidden_dim is specified, build the predictor head
        # the input dim of predictor is the output dim of the projector
    ...
    def _init_weights(self, m):
        ...
        if isinstance(m, nn.Linear): # Check if the layer is a linear layer
            trunc_normal_(m.weight, std=0.02) # Initialize weights with truncated normal distribution
            if isinstance(m, nn.Linear) and m.bias is not None:# If the layer has a bias term
    ...
    def _build_mlp(self, num_layers, input_dim, hidden_dim, output_dim, use_bn=False):
        ...
        build a mlp
        """
        mlp = [] # Initialize an empty list to hold MLP layers
        for layer in range(num_layers): # Loop over the number of layers,if layer=3,then 0,1,2

            #compute the input and out dimension of the layer
            # the structure: input_dim, hidden1, hidden2, ... output dim
            dim1 = input_dim if layer == 0 else hidden_dim
            # dim2 = output_dim if it is the last layer
            ...
            if layer < num_layers - 1:
                if use_bn:
                    mlp.append(nn.BatchNorm1d(dim2)) # Optionally add batch normalization
                mlp.append(nn.GELU()) # Add GELU activation

        return nn.Sequential(*mlp) # Return the MLP as a sequential module
...
class Group_Superivsion_Head(nn.Module):
    ...
    def __init__(
        ...
        ### the weight normalization of the last layer
